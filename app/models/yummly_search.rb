class YummlySearch
	...
	def self.all(options = {})
		response = get("/v1/api/recipes?_app_id=#{@app_id}&_app_key=#{@api_key}&maxResult=50", {query: options})
	...
	def self.find_by_ingredients(ingredients)
		...
		response = get("/v1/api/recipes?_app_id=#{@app_id}&_app_key=#{@api_key}&#{included_ingredients_string}&requirePictures=true&maxResult=50")
